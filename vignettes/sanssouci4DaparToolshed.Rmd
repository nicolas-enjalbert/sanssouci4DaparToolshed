...
```{r}
library(sanssouci)
sanssouci_object <- SansSouci(Y = SummarizedExperiment::assay(obj[[length(obj)]]), 
          groups = (design_qf(obj)$Condition == design_qf(obj)$Condition[1])*1)
```
...
Il faut également définir les interfaces d'utilisation du test de limma. 
...
rowTestLimma_1vs1 <- function(X, categ, alternative = c("two.sided")){
  ...
  for(b in seq(B)){
    # print(b)
    categ_b <- categ[,b]
    res <- limmaAnalyses_1vs1(Y = X, 
                              groups = categ_b)
    # print(head(res$P_Value))
...
```{r}
sanssouci_object_cal_limma <- fit(sanssouci_object, alpha = alpha, rowTestFUN = rowTestLimma_1vs1, verbose = TRUE, B = 1000)
...
```{r}
pval <- pValues(sanssouci_object_cal_limma)
logFC <- foldChanges(sanssouci_object_cal_limma)
```
...
volcanoPlot(sanssouci_object_cal_limma, p = pval_threshold, r = Foldchange_thlogFC)
```

La filtration permets de sélectionner 19 protéines. Parmis ces protéines, la garanties statistiques à 50% permet de dire qu'il y a au moins 15 protéines qui sont réellement différentiellement exprimées. Cela revient à dire que la proportion de faux positifs n'excède pas 21%.
...
names_prot <- rownames(obj[[length(obj)]])
...
df_sel <- data.frame(name_prot = names_prot, 
                     pval = as.vector(pval), 
                     logFC = as.vector(logFC))
pred_sel <- predict(sanssouci_object_cal_limma, S = which(sel_prot))
...
head(df_sel)
attributes(df_sel[[name_col_sel]])
```
