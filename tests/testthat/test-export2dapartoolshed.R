...
test_that("Unit/fctal test of export2dapartoolshed : good", {
  ...
  pval_thr <- 0.5
  logFC_thr <- 0.001
  res <- export2dapartoolshed(sanssouci_obj, pval_threshold = pval_thr,
                              Foldchange_thlogFC = logFC_thr)

  expect_true(is.data.frame(res))
  expect_equal(colnames(res), c("name_prot", "pval", "logFC", "sel_1"))
  expect_equal(names(attributes(res)), c("names", "row.names", "nb_permutation",
                                         "rowTestFun", "class"))
  ...
  expect_equal(attributes(res$sel_1),
               list("thr_pval" = pval_thr,
                    "thr_logFC" = logFC_thr,
                    "n_sel" = sum(S),
                    "FDP" = PHB[["FDP"]],
                    "TP" = PHB[["TP"]]))
  ...
  res1 <- export2dapartoolshed(qfeature_obj = qfeature_obj,
  ...
  pval_thr2 <- 0.05
  logFC_thr2 <- 0.04
  res2 <- export2dapartoolshed(qfeature_obj = res1 ,
                               sanssouci_obj = sanssouci_obj,
  ...
  expect_equal(attributes(res$sel_2),
               list("thr_pval" = pval_thr2,
...
test_that("Unit test of export2dapartoolshed : errors", {
  ...
  expect_error(export2dapartoolshed(sanssouci_obj = 10,
                       pval_threshold = 1,
                       Foldchange_thlogFC = 1),
               regexp = "sanssouci_obj must be a 'SansSouci' class object.")

  # Do not give a qfeatures object
  expect_error(export2dapartoolshed(sanssouci_obj = sanssouci_obj,
                                    qfeature_obj = 10,
                                    pval_threshold = 1,
                                    Foldchange_thlogFC = 1),
